...
float3 GerstnerOffset4 (float2 xzVtx, half4 steepness, half4 amp, half4 freq, half4 speed, half4 dirAB, half4 dirCD) {
    ...
    half4 dotABCD = freq.xyzw * half4(dot(dirAB.xy, xzVtx), dot(dirAB.zw, xzVtx), dot(dirCD.xy, xzVtx), dot(dirCD.zw, xzVtx));
        ...
        float4 TIME = _Lux_Time.xxxx * speed;
    #else
        float4 TIME = _Time.yyyy * speed;
    #endif
...
half3 GerstnerNormal4 (float2 xzVtx, half4 steepness, half4 amp, half4 freq, half4 speed, half4 dirAB, half4 dirCD) {
...
void Gerstner ( out float3 offs, out half3 nrml,
             float3 vtx, float3 tileableVtx, 
             half4 amplitude, half4 frequency, half4 steepness, 
             half4 speed, half4 directionAB, half4 directionCD ) {
    offs = GerstnerOffset4(tileableVtx.xz, steepness, amplitude, frequency, speed, directionAB, directionCD);
